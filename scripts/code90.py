...
# ============================================================
scores_par_match = {}
debug_scores = {"appel_ok": False, "code_http": None, "nb_evenements": 0, "erreur": None, "exemples_noms": []}
try:
    if API_KEY:
        rep_scores = requests.get(URL_SCORES, timeout=15)
        debug_scores["code_http"] = rep_scores.status_code
        if rep_scores.status_code == 200:
            debug_scores["appel_ok"] = True
            evenements = rep_scores.json()
            debug_scores["nb_evenements"] = len(evenements)
            for ev in evenements:
                home_s = ev.get("home_team")
                away_s = ev.get("away_team")
                if not home_s or not away_s:
                    continue
                if len(debug_scores["exemples_noms"]) < 5:
                    debug_scores["exemples_noms"].append(f"{home_s} vs {away_s}")
            ...
            debug_scores["erreur"] = rep_scores.text[:200]
