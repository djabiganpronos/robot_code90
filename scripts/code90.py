...
debug_scores = {"appel_ok": False, "nb_fixtures": 0, "erreur": None,
                 "errors_live": None, "errors_date": None, "results_live": None, "results_date": None}
...
if APIFOOTBALL_KEY:
    try:
        ...
        vus = set()
        for r, label in ((r1, "live"), (r2, "date")):
            if r.status_code == 200:
                data = r.json()
                erreurs = data.get("errors")
                debug_scores[f"errors_{label}"] = erreurs if erreurs else None
                debug_scores[f"results_{label}"] = data.get("results")
                ...
                debug_scores["erreur"] = f"HTTP {r.status_code} ({label}) : {r.text[:150]}"
